...
Calendar = (function () {
    function y(at) {
        at = at || {};
        this.args = at = al(at, {animation:!c, cont:null, bottomBar:true, date:true, fdow:aq("fdow"), min:null, max:null, reverseWheel:false, selection:[], selectionType:y.SEL_SINGLE, weekNumbers:false, align:"Bl/ / /T/r", inputField:null, trigger:null, dateFormat:"%Y-%m-%d", opacity:j ? 1 : 3, titleFormat:"%b %Y", showTime:false, timePos:"right", time:true, minuteStep:5, disabled:ad, checkRange:false, dateInfo:ad, onChange:ad, onSelect:ad, onTimeChange:ad, onFocus:ad, onBlur:ad});
        this.handlers = {};
    ...
    l.moveTo = function (aG, aD) {
        ...
        if (az.showTime) {
            this.setHours(aG.getHours());
            this.setMinutes(aG.getMinutes())
        }
        if (!az.animation) {
            aD = false
        }
        ...
        if (aD && !(aC == 0 && aD == 2)) {
            ...
            this._bodyAnim = ap({onUpdate:v(function (aK, aM) {
                var aL = ay(aK);
                if (P) {
    ...
    l.showAt = function (P, ax, av, pos) {
        ...
        if (av) {
            ...
            this._showAnim = ap({onUpdate:function (ay, az) {
    ...
    l.hide = function () {
        ...
        if (this.args.animation) {
            ...
            this._showAnim = ap({onUpdate:function (av, aw) {
    ...
    function K() {
        ...
        p(P.main, D);
        p([P.focusLink, P.yearInput], this._focusEvents = {focus:v(i, this), blur:v(e, this)});
    ...
    function u(ar, P) {
        ...
        if (!ar.args.animation) {
            ...
            ar._menuAnim = ap({onUpdate:function (au, av) {
    ...
    function I(az, ay) {
        ...
        if (au && !au.getAttribute("disabled")) {
            var D = au.getAttribute("dyc-btn"), ax = au.getAttribute("dyc-type"), av = au.getAttribute("dyc-date"), at = this.selection, ar, P = {mouseover:an, mousemove:an, mouseup:function (aC) {
    ...
    function ab(D, ax) {
        ...
        aE = {"%a":aq("sdn")[aA], "%A":aq("dn")[aA], "%b":aq("smn")[P], "%B":aq("mn")[P], "%C":1 + Math.floor(ay / 100), "%d":aw < 10 ? "0" + aw : aw, "%e":aw, "%H":aB < 10 ? "0" + aB : aB, "%I":au < 10 ? "0" + au : au, "%j":aD < 10 ? "00" + aD : aD < 100 ? "0" + aD : aD, "%k":aB, "%l":au, "%m":P < 9 ? "0" + (1 + P) : 1 + P, "%o":1 + P, "%M":at < 10 ? "0" + at : at, "%n":"\n", "%p":ar ? "PM" : "AM", "%P":ar ? "pm" : "am", "%s":Math.floor(D.getTime() / 1000), "%S":av < 10 ? "0" + av : av, "%t":"\t", "%U":az < 10 ? "0" + az : az, "%W":az < 10 ? "0" + az : az, "%V":az < 10 ? "0" + az : az, "%u":aA + 1, "%w":aA, "%y":("" + ay).substr(2, 2), "%Y":ay, "%%":"%"};
    ...
    function ap(D, aw, ar) {
        D = al(D, {fps:50, len:15, onUpdate:ad, onStop:ad});
        if (j) {
        ...
        av();
        return{start:av, stop:P, update:au, args:D, map:at}
    }

    var Z = {elastic_b:function (D) {
    ...
    function J(au) {
        ...
        av = {x:au.offsetLeft - D, y:au.offsetTop - at};
        if (au.offsetParent) {
            P = J(au.offsetParent);
            av.x += P.x;
            av.y += P.y
