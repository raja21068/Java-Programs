 ...
 */
    ...
    class adminPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`admin`');
            $field = new IntegerField('admin_id', null, null, true);
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new StringField('user_name');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new IntegerField('password');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('adminssearch', $this->dataset,
                array('admin_id', 'user_name', 'password'),
                array($this->RenderText('Admin Id'), $this->RenderText('User Name'), $this->RenderText('Password')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('adminasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('admin_id', $this->RenderText('Admin Id')));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $column = new TextViewColumn('admin_id', 'Admin Id', $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'admin_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'adminGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new adminPage("admin.php", "admin", GetCurrentUserGrantForDataSource("admin"), 'UTF-8');
        $Page->SetShortCaption('Admin');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Admin');
