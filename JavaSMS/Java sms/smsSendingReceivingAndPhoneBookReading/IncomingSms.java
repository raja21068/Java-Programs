...
public class IncomingSms{
	...
	public IncomingSms(String smspdu) throws PduParseException{
		...
		try{
			...
			if (octet1 > 0)
			{
				...
				smscAddress = PduCodec.swapDigits(tmpstr);
			...
			senderAddress = PduCodec.swapDigits(tmpstr);
			senderAddress = senderAddress.substring(0, senderAddressLength);
			
			// 1001xxxx?
			if ((senderAddressType & 0xf0) == 0x90) //  & senderAddressLength>5
			...
			if (sms_format.equals("incoming"))
			{
				...
				time = PduCodec.TimeStampDecode(tpScts);
				i = i + 7 * 2;
			}
			...
			if ((tpDcs & 4) == 0) msg = PduCodec.sevenBitDecode(tpUd, tpUdl);
			else msg = PduCodec.eightBitDecode(tpUd);
		}
		...
		if (sim_data_offset != -1) msg = msg.substring(sim_data_offset + 16, msg.length());
		if ((i + tpUd.length()) < pdulen) throw new PduParseException("PDU too long");
	}
	...
	public String toString()
	{
		...
		sb.append("\n type             = "+ sms_format);
		sb.append("\n smscAddress      = " + smscAddress);
		sb.append("\n smsDeliverCode   = 0x" + PduCodec.toHexString(smsDeliverCode));
		sb.append("\n senderAddress    = " + senderAddress);
		sb.append("\n tpPid            = 0x" + PduCodec.toHexString(tpPid));
		sb.append("\n tpDcs            = 0x" + PduCodec.toHexString(tpDcs));
		
		if (sms_format.equals("incoming"))
			sb.append("\n Time/Date        = " + PduCodec.displayTimeDate(time));
