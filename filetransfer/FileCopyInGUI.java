...
public class FileCopyInGUI extends JFrame implements ActionListener{
	...
	void save(){
		...
		String str=dialog.getDirectory() + dialog.getFile();
		d.setText("");	
		if(!str.equals("nullnull"))d.setText(str+fileName);
	...
	class startThread extends Thread{
		public void run(){
			...
			try{
				...
				RandomAccessFile wr=new RandomAccessFile(desti,"rw");
			    ...
			    bar.setMaximum(totalPackets+1);
			    bar.setMinimum(0);
			    bar.setValue(initialsize/10000);
			    out.skipBytes(initialsize);
			    wr.seek(initialsize);
			    
			    for(i=(initialsize/10000)+1; i<=totalPackets; i++){
			    	bar.setValue(i);
			         bar.validate();
