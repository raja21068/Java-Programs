<table border=2 width="500px">
    <tr>
        ...
        if (isset($_REQUEST['day_id']) && ($_REQUEST['day_id']+=0) > 0)
            $day_id = $_REQUEST['day_id'];
        else
            $day_id = 'day_id';
        if (isset($_REQUEST['time_id']) && ($_REQUEST['time_id']+=0) > 0)
        ...
        while ($row = mysqli_fetch_array($time_table)) {
            $time_table_row[$row['days_name']][$row['time_id']] = $row['course_name'] . '<br>Room: ' . $row['room_name'] . '<br>' . $row['name'];
        }
        foreach ($time_row as $j => $row) {
            echo "<td style='font-weight: bold'>" . $row['time'] . "</td>";
        }
