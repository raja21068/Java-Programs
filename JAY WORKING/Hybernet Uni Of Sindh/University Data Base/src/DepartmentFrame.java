...
public class DepartmentFrame extends javax.swing.JFrame {
    ...
    private void addBtnActionPerformed(java.awt.event.ActionEvent evt)throws Exception {//GEN-FIRST:event_addBtnActionPerformed
        ...
        boolean check = false;
        int facID = faculty.getFactId();
        String depName = departmentTextField.getText();
        Department depBean = new Department(facID,depName,remarksTextField.getText());
        
        java.util.ArrayList v = DatabaseManager.getDepartment(facID);
        for(int i =0 ; i<v.size(); i++){
            Department dBean = (Department) v.get(i);
        ...
        if(check){
            ...
            DatabaseManager.addDepartment(depBean.getFactId(),depBean.getDeptName(),depBean.getRemarks());
    ...
    private void DelBtnActionPerformed(java.awt.event.ActionEvent evt)throws Exception {
        ...
        Department dBean = (Department)departmentList.getSelectedValue();
        DatabaseManager.deleteDepartment(dBean.getDeptID());
    ...
    private void updateBtnActionPerformed(java.awt.event.ActionEvent evt)throws Exception {
        ...
        DatabaseManager.updateDepartment(facID,Integer.parseInt(departmentIdTextField.getText()),departmentTextField.getText(), remarksTextField.getText());
        javax.swing.JOptionPane.showMessageDialog(this, departmentTextField.getText()+" is updated...");
        refreshDepartmentList();
    }
    ...
    private void setAllFacultyInComboBox() {
        try {
            ...
            java.util.ArrayList v = DatabaseManager.getFaculty();
            for(int i=0 ; i<v.size() ; i++){
                Faculty fBean = (Faculty)v.get(i);
    ...
    private void refreshDepartmentList()throws Exception{
        Faculty facBean = (Faculty) facultyComboBox.getSelectedItem();
        if(facBean==null)return;
        java.util.ArrayList vector = DatabaseManager.getDepartment(facBean.getFactId());
        departmentList.removeAll();
        departmentList.setListData(vector.toArray());
        clear();
    }
