/*
* NEW BrowserCompAPI version 1.0.2.24
...
if (!isNotSmartbar() && navigator.userAgent.indexOf("Firefox") != -1 && (window != window.top) && !getContext(window.conduitWindowName) && typeof (abstractionlayer) === "undefined") {
...
if (navigator.userAgent.indexOf("Chrome") != -1 && (document.location.href.indexOf("#mam_extension") != -1 || document.location.href.indexOf("#mam_app") != -1)) {
...
break;
case "extensionButtonClicked":
if (isToMe) {
if ("EBMAMButtonClicked" in window) {
EBMAMButtonClicked();
}
}
break;
...
if (communicate.isSupportedFunction("isExtensionInstalled")) {
communicate.isExtensionInstalled(extenstionId, callback);
} else {
if (callback) {
callback(UNSUPPORTED);
} else {
...
return communicator.injectScript2(window, strScript);
...
"platform": { "isToolbarVisible": "", "getToolbarBornDate": "", "onChangeViewState": { "addListener": "eventHandler"}, "getToolbarVersion":"" ,"executeExternalProgram": "registeredName, parameters", "getAppsList": "", "onToolbarReady": { "addListener": "eventHandler"}, "getInfo": "", "isAppInstalled": "appId", "refresh": "forceRefresh", "refreshApp": "appId", "setSkin": "settings", "search": { "getTerm": "", "onExecuted": { "addListener": "eventHandler" }, "onTextChanged": { "addListener": "eventHandler"} }, "getScreenHeight": "", "getScreenWidth": "", "openApp": "guid" },
...
storageObj.get = function (name, prefix, type, decryptVal, callback) {
var data = undefined;
try {
...
data = abstractionlayer.commons.repository.getKey(getStorageKey(prefix, name), false); //for version < 10.13 get the key as plain test (w/o decryption)
}
else {
...
storageObj.remove = function (name, prefix, type, data) {
if (type == "item") {
...
conduit.storage.get = function (name, prefix, type, decryptVal, callback) {
...
if (decryptVal === undefined) {
decryptVal = true; //by default decrypt all values
}
var data = JSON.parse(window.external.invokePlatformActionLimitedSync(21, absAPI, decryptVal, getStorageKey(prefix, name))).result;
if (callback) callback(data);
return data;
}
conduit.storage.set = function (name, prefix, type, data) {
if (type == "key") {
window.external.invokePlatformActionLimitedSync(21, 1, true, getStorageKey(prefix, name), data);
if (prefix == "global") {
conduit.advanced.messaging.postTopicMessage("EBGlobalKeyChanged", JSON.stringify({ keyName: name, keyValue: data }));
}
else if (typeof (EBKeyChanged) === "function") { //this is app key
EBKeyChanged(name, data);
}
}
else if (type == "item") {
window.external.invokePlatformActionLimitedSync(21, 3, getStorageKey(prefix, name), data);
}
}
conduit.storage.remove = function (name, prefix,type) {
if (type == "key") {
window.external.invokePlatformActionLimitedSync(21, 6, getStorageKey(prefix, name));
}
else {
window.external.invokePlatformActionLimitedSync(21, 7, getStorageKey(prefix, name));
}
}
}
...
key = "_LOCAL_" + key + "_" + conduit.currentApp.appId;
}
return key;
}
conduit.storage.get = function (name, prefix, type, decryptVal, callback) {
...
if (data) {
data = JSON.parse(data);
//data was found in the old bar storage, save it to the SB repository
conduit.storage.set(name, prefix, type, data);
return data; //return the data at this point since there is no need to do unescape for the old bar data
}
}
}
...
//for backward, remove first the key from the old bar storage and set the new one
if (checkLocalStorageAvailbility()) {
...
this.deleteKey = function (key) {
//for backward to previous versions, delete also the key with the old prefix.
conduit.storage.remove(key, "appID", "key");
//and remove also the key with the current prefix
conduit.storage.remove(key, "appDomain", "key");
};
this.deleteFile = function (key) {
conduit.storage.remove(key, "appID", "item");
};
this.deleteGlobalKey = function (key) {
conduit.storage.remove(key, "global", "key");
};
...
this.loadFile = function (name) { return conduit.storage.get(name, "appID", "item") || null; };
...
allowScrolls: (navigator.userAgent.indexOf("MSIE") != -1)?{hScroll: !!featuresObj.hscroll , vScroll: !!featuresObj.vscroll }:{hScroll: featuresObj.hscroll !== false, vScroll: featuresObj.vscroll !== false},
...
var data = conduit.storage.get(keyName, "appDomain", "key");
if (!data) { //for backward, if the key wasn't found with the app domain prefix try to look for it using the app ID prefix (which was the previous prefix)
data = conduit.storage.get(keyName, "appID", "key", false); //when using the previus prefix no need to decrypt the value.
if (data) { //if the key now found, save it to the repository encrypted with the correct app domain prefix and remove the old one
conduit.storage.set(keyName, "appDomain", "key", data);
conduit.storage.remove(keyName, "appID", "key");
}
}
return data || "";
};
this.retrieveGlobalKey = function (keyName) {
return conduit.storage.get(keyName, "global", "key") || "";
};
...
this.storeKey = function (key, value) {
//for backward, remove first if the key with the previous prefix (appID) and set the new one
conduit.storage.remove(key, "appID", "key");
conduit.storage.set(key, "appDomain", "key", value);
};
this.saveFile = function (name, content) {
conduit.storage.set(name, "appID", "item", content);
};
this.setLinksTarget = function (target) {
...
this.storeGlobalKey = function (key, value) {
conduit.storage.set(key, "global", "key", value);
};
this.showToolbar = function(callback){
var callbackFunction = createCBFunction(callback);
...
function dispatchDocComplete(url, tabId) {
var tab = { url: url, id: tabId };
...
conduit.tabs.__dispatchDocComplete(url, tabId);
}
