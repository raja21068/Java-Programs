...
--src  (Contains a NetBeans project for each sample)
    ...
    --<Sample2>
        ...
--web-files (Contains Deployment Toolkit files so you can run the 
                samples offline)
<sample1>.html  (Runs the sample embedded in a browser)
<sample1>.jar   (Runs the sample as a standalone application)
<sample1>.jnlp  (Deployment descriptor for applications embedded in the 
                browser and Web Start applications)
<sample2>.html
 ...
...
- If you run the applications in the browser, a supported browser**.
...
You can run the samples in any of three ways:
...
To run as a Web Start application, do either of the following:

- Double-click the JNLP file.
- Double-click the HTML file, then click the link that says Click to 
  launch this app as webstart. This link is controlled with JavaScript 
  code that engages the Java Deployment Toolkit, which ensures the user 
  environment has the required Java and JavaFX runtimes.

You can also package any sample as a native bundle that includes an
installer and a copy of the JRE for execution in an environment that
does not have JavaFX installed. See "Deploying JavaFX Applications" at
