...
public class MonthlyChart {
    ...
    JFreeChart chart ;
    YearBean yearbean; 
    public MonthlyChart(YearBean yearbean){
         this.yearbean =  yearbean; 
        set = new DefaultCategoryDataset();
        setDataset(set , yearbean);
        setChart();
    }
...
//      CategoryPlot p = (CategoryPlot)chart.getPlot();
        ...
        CategoryAxis domainAxis = new CategoryAxis(""+ yearbean.getYear());
        CombinedCategoryPlot plot = new CombinedCategoryPlot(domainAxis, new NumberAxis("Rupess"));
        plot.add(subplot1, 2);
        JFreeChart chart = new JFreeChart(plot);
        
        frame = new ChartFrame("Monthwise chart of "+ yearbean.getYear() , chart);
        frame.setSize(500,550);
    }
    private void setDataset(DefaultCategoryDataset dataset , YearBean yb){
        ...
        try{
            ...
            for(int i=0;i<sales.size();i++){
                ...
                if(salePurchase.getType()==Constant.SALE){
                    String month = (DateFormatter.dateToStringMonthFormat(salePurchase.getDay()).split("-"))[0];
